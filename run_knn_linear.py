...
with open('results-linear.csv', 'a') as csvfile:
	writer = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
	for samples in X_LIST:
		for features in Y_LIST:
			for noisy in NOISE_LIST:
				for k in N_NEIGH:
					x_path = str(samples) + "/x-" + str(samples) + "-" + str(features) + "-" + str(noisy) +".txt"
					y_path = str(samples) + "/y-" + str(samples) + "-" + str(features) + "-" + str(noisy) +".txt"
